...
# below are totalizes variables, they will count the numbers of each type organized.
total_documents = 0
total_music = 0
total_videos = 0
total_images = 0
...
def createFolders():
    """
    This is the function that creates a new folder named
    ...
    - parameters: This function does not receive any parameter
    ...
    for i in os.listdir(os.getcwd()):  # Iterate through the folder
        if os.path.isfile(i):  # If the i is file, add +1 to total
            total = total + 1
    if total > 1:  # if total is greater than 1, it means that except the PowFu File Organizer, and runner.bat there are files to organize
        if os.path.exists(main_folder):  # if the the folder where we will organize already existis
...
def moveFiles(_file, _destination):
    ...
    destin = os.path.join(os.getcwd(), main_folder, _destination)
    source = os.path.join(os.getcwd(), _file)  # source path, in this case where the scrip will be executed
    try:
        # if the file is already in the folder, unlike it and move a new one.
        if _file in os.listdir(destin):
            os.unlink(os.path.join(destin, _file))
...
flag = createFolders()  # here we instance a variable name as flag and consequently call the createFolder function
...
for file in os.listdir(os.getcwd()):  # iterate again all the files
    ...
    if os.path.isfile(file):  # if it is a file
        ...
        # the os.path.splitext() method return a tuple of a path like ("C", "Users", "AntonioDev", "somefile", ".extension of the file")
        ...
        if file.endswith(extension):  # if the current file ends with some extension
            ...
            # the same happens for others files found.
                ...
                if file == "PowFu-File Organizer.exe":
                    pass
                else:
...
if flag == 0:  # if flag is equal to zero, then we have complete all the task successfully, show the results to the user.
    print("==============RESULTS========================")
